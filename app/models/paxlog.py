...
class PaxGroup(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (Index("idx_pax_groups_entity", "entity_id"),)

    entity_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("entities.id"), nullable=False)
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    company_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tiers.id"))
...
class CredentialType(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    booking_service_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("departments.id"))
    active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
...
class ComplianceMatrixEntry(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    scope: Mapped[str] = mapped_column(String(30), nullable=False, default="all_visitors")
    defined_by: Mapped[str] = mapped_column(String(20), nullable=False)
    set_by: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
...
class Ads(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "visit_category IN ('project_work','maintenance','inspection','visit','permanent_ops','other')",
    ...
    planner_activity_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("planner_activities.id"))
    ...
    requester_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    ...
    visit_category_requires_planner: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    cross_company_flag: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    is_round_trip_no_overnight: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    submitted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    approved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    rejected_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    rejection_reason: Mapped[str | None] = mapped_column(Text)

    pax_entries: Mapped[list["AdsPax"]] = relationship(back_populates="ads", cascade="all, delete-orphan")
...
class AdsAllowedCompany(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    company_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("tiers.id"), nullable=False)
...
class AdsPax(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    user_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
    contact_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tier_contacts.id"))
    status: Mapped[str] = mapped_column(String(30), nullable=False, default="pending_check")
    ...
    booking_request_sent: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    current_onboard: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
...
class PaxIncident(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    contact_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tier_contacts.id"))
    company_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tiers.id"))
    pax_group_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("pax_groups.id"))
    ...
    recorded_by: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
...
class MissionNotice(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    eligible_displacement_allowance: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    mission_type: Mapped[str] = mapped_column(String(50), nullable=False, default="standard")
...
class MissionProgram(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    site_asset_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("ar_installations.id"))
    planned_start_date: Mapped[date | None] = mapped_column(Date)
    planned_end_date: Mapped[date | None] = mapped_column(Date)
    project_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("projects.id"))
    generated_ads_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("ads.id"))
    notes: Mapped[str | None] = mapped_column(Text)
...
class MissionPreparationTask(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    assigned_to_user_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
    linked_ads_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("ads.id"))
...
class MissionStakeholder(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    notification_level: Mapped[str] = mapped_column(String(20), nullable=False, default="summary")
...
class ExternalAccessLink(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    ads_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("ads.id"), nullable=False)
    ...
    otp_required: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true", nullable=False)
    ...
    otp_attempt_count: Mapped[int] = mapped_column(SmallInteger, default=0, server_default="0", nullable=False)
...
class PaxRotationCycle(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    user_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
    contact_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tier_contacts.id"))
    site_asset_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("ar_installations.id"), nullable=False)
    ...
    auto_create_ads: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true", nullable=False)
    ads_lead_days: Mapped[int] = mapped_column(SmallInteger, default=7, server_default="7", nullable=False)
    default_project_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("projects.id"))
    default_cc_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("cost_centers.id"))
    created_by: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class ProfileType(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("entity_id", "code", name="uq_profile_type_entity_code"),)
...
class PaxProfileType(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    user_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"))
    contact_id: Mapped[PyUUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("tier_contacts.id", ondelete="CASCADE")
    )
    profile_type_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("profile_types.id"), nullable=False)
    assigned_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    assigned_by: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
...
class PaxCompanyGroup(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (Index("idx_pax_company_groups_tiers", "tiers_id"),)

    entity_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("entities.id"), nullable=False)
    tiers_id: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("tiers.id"), nullable=False)
    group_name: Mapped[str] = mapped_column(String(200), nullable=False)
    supervisor_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
    active: Mapped[bool] = mapped_column(Boolean, default=True, server_default="true", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
