...
async def list_project_teams(
    ...
    """Liste les equipes attachees a un projet, avec hydratation (nom, taille)."""
    from app.models.teams import ProjectTeam, Team, TeamMember
    from sqlalchemy import func as _func

    # Verifie le projet existe + appartient a l'entite
    proj = (await db.execute(
        select(Project).where(Project.id == project_id, Project.entity_id == entity_id)
    )).scalar_one_or_none()
    if not proj:
    ...
    rows = (await db.execute(
        ...
        .where(ProjectTeam.project_id == project_id, Team.deleted_at.is_(None))
        .order_by(ProjectTeam.attached_at.desc())
    )).all()
    out = []
    for pt, team in rows:
        ...
        out.append({
            ...
            "role": pt.role,
            "attached_at": pt.attached_at.isoformat() if pt.attached_at else None,
            "attached_by": str(pt.attached_by) if pt.attached_by else None,
            "team_name": team.name,
...
async def attach_team_to_project(
    project_id: UUID,
    body: dict,
    entity_id: UUID = Depends(get_current_entity),
    current_user: User = Depends(get_current_user),
    ...
    """Attache une equipe au projet. Idempotent si deja attachee."""
    from app.models.teams import ProjectTeam, Team
    ...
    # Idempotent : si deja attache, renvoie l'existant
    existing = (await db.execute(
    ...
    if existing:
        ...
        return {
            "id": str(existing.id), "project_id": str(existing.project_id),
            "team_id": str(existing.team_id), "role": existing.role,
            "attached_at": existing.attached_at.isoformat() if existing.attached_at else None,
            "team_name": team.name, "team_visibility": team.visibility,
        }

    pt = ProjectTeam(
        project_id=project_id, team_id=team_id,
        role=role, attached_by=current_user.id,
    )
    db.add(pt)
    await db.commit()
    await db.refresh(pt)
    return {
        "id": str(pt.id), "project_id": str(pt.project_id),
        "team_id": str(pt.team_id), "role": pt.role,
        "attached_at": pt.attached_at.isoformat() if pt.attached_at else None,
        "team_name": team.name, "team_visibility": team.visibility,
    }
...
async def detach_team_from_project(
    project_id: UUID,
    team_id: UUID,
    entity_id: UUID = Depends(get_current_entity),
    _: None = require_permission("project.update"),
    ...
    """Detache une equipe du projet (suppression hard, pas d'historique fin ici)."""
    from app.models.teams import ProjectTeam
