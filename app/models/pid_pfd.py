...
class PIDDocument(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    # Relationships
    revisions: Mapped[list["PIDRevision"]] = relationship(back_populates="pid_document", cascade="all, delete-orphan")
    equipment_items: Mapped[list["Equipment"]] = relationship(back_populates="pid_document")
    connections: Mapped[list["PIDConnection"]] = relationship(
        back_populates="pid_document", cascade="all, delete-orphan"
    )
    dcs_tags: Mapped[list["DCSTag"]] = relationship(back_populates="pid_document")
    lock: Mapped["PIDLock | None"] = relationship(back_populates="pid_document", uselist=False)
...
class Equipment(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    operating_pressure_barg: Mapped[Decimal | None] = mapped_column(Numeric, nullable=True)
    operating_temperature_c: Mapped[Decimal | None] = mapped_column(Numeric, nullable=True)
    ...
    mxgraph_cell_id: Mapped[str | None] = mapped_column(String(100), nullable=True)  # draw.io sync
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    ...
    # Relationships
    pid_document: Mapped["PIDDocument | None"] = relationship(back_populates="equipment_items")
...
class PIDConnection(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    continuation_ref: Mapped[str | None] = mapped_column(String(100), nullable=True)  # cross-PID reference
    flow_direction: Mapped[str] = mapped_column(String(20), nullable=False, default="forward")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class DCSTag(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    equipment_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("equipment.id"), nullable=True)
    pid_document_id: Mapped[PyUUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("pid_documents.id"), nullable=True
    )
...
class TagNamingRule(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (Index("idx_tag_naming_rules_entity", "entity_id"),)
    ...
    applies_to_types: Mapped[list] = mapped_column(JSONB, nullable=False, default=list, server_default="'[]'::jsonb")
...
class PIDLock(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("pid_document_id", name="uq_pid_locks_document"),)
    ...
    locked_by: Mapped[PyUUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
