...
class ComplianceRule(UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    target_type: Mapped[str] = mapped_column(String(30), nullable=False)  # tier_type, tier_tag, person_tag, asset, department, job_position, all
...
# ─── Mobile device pairing (QR login, WhatsApp-Web style) ────────────────────

class MobilePairingToken(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """
    Short-lived token generated by the web (for a logged-in user) to pair
    a mobile device without typing credentials. The mobile scans a QR
    containing this token and exchanges it for JWT tokens.

    Security:
      - Plaintext token is NEVER stored. Only the SHA-256 hash.
      - Single-use (status transitions pending -> consumed | expired | revoked).
      - Short TTL (~2 minutes).
      - Rate-limited per user (see auth.mobile_pair.rate_limit setting).
    """
    __tablename__ = "mobile_pairing_tokens"
    __table_args__ = (
        Index("ix_mobile_pairing_token_hash", "token_hash", unique=True),
        Index("ix_mobile_pairing_user_status", "user_id", "status"),
    )
    ...
    # Consumption context (mobile) — filled when status goes pending -> consumed
    consumed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    consumed_ip: Mapped[str | None] = mapped_column(String(45), nullable=True)
    consumed_device_info: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    # Lifecycle
...
class UserVerification(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """
    Evidence that a user has verified an attribute (phone, email, location,
    ID document, biometric). Rows are append-only — a new verification
    supersedes but doesn't delete previous ones, giving us an audit trail.

    `type` values:
      - email       : OTP email confirmed
      - phone       : OTP SMS/WhatsApp confirmed
      - location    : GPS coordinates declared from a trusted device
      - id_document : photos of ID + selfie, pending operator review
      - biometric   : local biometric enrolled and bound to user_id
    """
    __tablename__ = "user_verifications"
    __table_args__ = (
        Index("ix_user_verification_user_type", "user_id", "type"),
        Index("ix_user_verification_status", "status"),
    )
    ...
    # otp_sms | otp_whatsapp | otp_email | gps | photo | face_id | fingerprint | operator_review
    evidence: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    # {otp_hash, gps_coords, attachment_ids, device_id, ...}

    verified_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    verified_by_user_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    rejection_reason: Mapped[str | None] = mapped_column(String(500), nullable=True)

    # Optional reference back to the specific phone/email row being verified
    target_phone_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
    target_email_id: Mapped[PyUUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
