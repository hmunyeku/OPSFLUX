...
export function AgentIaTab() {
  ...
  const handleSave = async () => {
    try {
      ...
      toast({ title: 'Configuration agent sauvegardée', variant: 'success' })
    } catch (err: unknown) {
  ...
  const handleSendDigestNow = async () => {
    ...
    try {
      ...
      toast({
        title: 'Digest envoyé',
        description: `Expédié à ${data.sent_to}`,
        variant: 'success',
      })
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail
      toast({ title: 'Envoi impossible', description: msg || String(err), variant: 'error' })
    } finally {
  ...
  if (isLoading || !config) {
    return <div className="p-4 flex items-center gap-2 text-xs text-muted-foreground"><Loader2 size={12} className="animate-spin" /> Chargement…</div>
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold flex items-center gap-2"><Bot size={14} /> Agent de maintenance IA</h3>
          <p className="text-xs text-muted-foreground">
            Résolution autonome de tickets via Claude Code/Codex — avec human-in-the-loop.
          </p>
        </div>
      ...
      {/* Activation */}
      <Section title="Activation" icon={<Bot size={12} />}>
        <Row label="Agent activé" hint="Désactive complètement l'agent. Les runs en cours ne sont pas interrompus.">
          <Toggle checked={Boolean(form.enabled)} onChange={(v) => set('enabled', v)} />
        </Row>
        <Row label="Mode d'autonomie par défaut">
          <select className="gl-form-input" value={String(form.default_autonomy_mode ?? 'recommendation')} onChange={(e) => set('default_autonomy_mode', e.target.value)}>
            <option value="observation">Observation (analyse seulement, pas de PR)</option>
            <option value="recommendation">Recommendation (PR draft, merge manuel)</option>
            <option value="autonomous_with_approval">Autonomous (staging + verification, puis approbation)</option>
          </select>
        </Row>
      </Section>

      {/* Connecteurs */}
      <Section title="Connecteurs par défaut">
        <Row label="GitHub">
      ...
      <Section title="Mode automatique" icon={<Clock size={12} />}>
        <Row label="Lancement auto sur nouveaux tickets" hint="Si activé, le scheduler scanne les tickets toutes les 5 min et lance l'agent sur les éligibles (dans la fenêtre).">
          <Toggle checked={Boolean(form.automatic_trigger_enabled)} onChange={(v) => set('automatic_trigger_enabled', v)} />
        </Row>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="gl-label-sm">Fenêtre — heure début (UTC)</label>
          ...
          <div>
            <label className="gl-label-sm">Fenêtre — heure fin (UTC)</label>
        ...
        <p className="text-[10px] text-muted-foreground">
          Laisse vide pour tourner 24/7. Wrap-around supporté : 23 → 6 = la nuit.
        </p>
        <Row label="Max runs par fenêtre (cap dur)">
        ...
        <Row label="Autoriser le déploiement direct (B/C)" hint="⚠ Attention : modes B/C déploient sans staging préalable.">
          <Toggle checked={Boolean(form.allow_direct_deployment)} onChange={(v) => set('allow_direct_deployment', v)} />
        </Row>
      </Section>

      {/* Rapport mail matinal */}
      <Section title="Rapport quotidien" icon={<Mail size={12} />}>
        <Row label="Email destinataire">
      ...
      <Section title="Budgets & sécurité" icon={<Shield size={12} />}>
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            <label className="gl-label-sm">Runs simultanés max</label>
          ...
          <div>
            <label className="gl-label-sm">Lignes max/run</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="gl-label-sm">Circuit breaker — seuil</label>
          ...
          <div>
            <label className="gl-label-sm">Cooldown (heures)</label>
      ...
      {/* Forbidden paths */}
      <Section title="Chemins interdits en modification" icon={<ListX size={12} />}>
        <textarea
        ...
        />
        <p className="text-[10px] text-muted-foreground">
          Un pattern glob par ligne. Tout chemin qui matche sera refusé au post-exec gate.
          L'agent ne pourra jamais modifier ces fichiers.
        </p>
      </Section>
