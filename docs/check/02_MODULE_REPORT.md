...
CREATE INDEX idx_documents_tenant_status ON documents(tenant_id, status, bu_id);
...
async def export_pdf(document_id: str, revision_id: str, tenant_id: str) -> bytes:
    """Génère un PDF via Puppeteer headless."""
    doc = await get_document(document_id, tenant_id)
    revision = await get_revision(revision_id)
    template = await get_template(doc.doc_type.default_template_id, tenant_id)
...
async def get_document(doc_id: str, request: Request):
    doc = await report_service.get_document(doc_id, request.state.tenant_id)
...
async def save_draft(doc_id: str, body: RevisionDraft, request: Request):
...
async def submit_for_validation(doc_id: str, body: SubmitRequest, request: Request):
...
async def approve_document(doc_id: str, body: ApproveRequest, request: Request):
    return await report_service.approve_document(
        doc_id=doc_id, comment=body.comment,
        tenant_id=request.state.tenant_id, actor_id=request.state.user_id,
    )

@router.post("/{doc_id}/reject", dependencies=[requires_permission("document.approve")])
async def reject_document(doc_id: str, body: RejectRequest, request: Request):
...
async def export_pdf(doc_id: str, revision_id: Optional[str] = None, request: Request = None):
    pdf_bytes = await report_service.export_pdf(
        doc_id, revision_id, request.state.tenant_id
    )
    filename = f"{(await get_document(doc_id, request.state.tenant_id)).number}.pdf"
    return Response(
        content=pdf_bytes,
...
async def export_docx(doc_id: str, request: Request):
    docx_bytes = await report_service.export_docx(doc_id, request.state.tenant_id)
...
## 9. PDCA — Phase ReportEditor (Phase 4)
