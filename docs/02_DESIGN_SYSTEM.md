...
17. [DataTable — Composant universel](#17-datatable--composant-universel)
18. [Page Layouts — Patterns standard](#18-page-layouts--patterns-standard)
19. [Filtres — Niveaux par module](#19-filtres--niveaux-par-module)
20. [Loading States & Skeletons](#20-loading-states--skeletons)
21. [Confirmation d'actions — Inline Confirm](#21-confirmation-dactions--inline-confirm)
22. [Toasts](#22-toasts)
23. [États de formulaire](#23-états-de-formulaire)
24. [Panneau Dynamique — Comportement Pin](#24-panneau-dynamique--comportement-pin)
25. [Raccourcis clavier](#25-raccourcis-clavier)
26. [Settings — Saving Pattern (Pajamas)](#26-settings--saving-pattern-pajamas)
27. [Quick Entry — Pattern ERPNext adapté](#27-quick-entry--pattern-erpnext-adapté)
28. [Inline Editing — Édition directe sans mode "Modifier"](#28-inline-editing--édition-directe-sans-mode-modifier)
29. [Workflow Editor — React Flow](#29-workflow-editor--react-flow)
30. [BlockNote — Custom Blocks](#30-blocknote--custom-blocks)
31. [Template Builder — Interface](#31-template-builder--interface)
32. [form_data — Collaboration via Y.Map](#32-form_data--collaboration-via-ymap)
33. [useUserPreference — localStorage + sync DB](#33-useuserpreference--localstorage--sync-db)
34. [Recherche globale — Full-text + pgvector](#34-recherche-globale--full-text--pgvector)
35. [**Patterns UI implémentés — Référence pratique**](#35-patterns-ui-implémentés--référence-pratique)
...
async def global_search(
    ...
    full_text_task = asyncio.create_task(
        _full_text_search(query, entity_id, bu_id, user_permissions, limit)
    )
    semantic_task = asyncio.create_task(
        _semantic_search(query, entity_id, bu_id, user_permissions, limit // 2)
    )
...
async def _full_text_search(query, entity_id, bu_id, permissions, limit) -> list:
    ...
    if "document.read" in permissions:
        docs = await db.execute(text("""
            ...
            ORDER BY score DESC
            LIMIT :lim
        """), {"q": query, "tid": entity_id, "bu": bu_id, "lim": limit // 3})
    ...
    if "asset.read" in permissions:
        assets = await db.execute(text("""
            SELECT 'asset' as object_type, id::text as object_id,
                   ...
                   ts_rank(search_vector, plainto_tsquery('french', :q)) as score
            FROM assets
            WHERE entity_id = :tid AND is_active = TRUE
              AND search_vector @@ plainto_tsquery('french', :q)
            ORDER BY score DESC LIMIT :lim
        """), {"q": query, "tid": entity_id, "lim": limit // 3})
...
async def _semantic_search(query, entity_id, bu_id, permissions, limit) -> list:
    """pgvector — recherche sémantique sur les chunks indexés."""
    embedding = await ai_service.embed(query, entity_id)
...
## UX Patterns — Règles obligatoires pour tous les formulaires et listes

> **Ces patterns sont obligatoires sur TOUTES les pages de l'application.**
> Tout développeur (humain ou IA) doit les respecter systématiquement.

### 1. Recherche unique contextualisable

**Règle** : Il n'y a qu'**une seule barre de recherche** dans toute l'interface — celle de la **Topbar**.
Les pages ne doivent **jamais** avoir leur propre barre de recherche.

- La barre de recherche du Topbar est un `<input>` réel (pas un bouton)
- Le placeholder s'adapte automatiquement à la page active : `"Rechercher — Assets..."`, `"Rechercher — Tiers..."`, etc.
- La valeur est stockée dans `useUIStore.globalSearch`
- Chaque page lit `globalSearch` et le passe comme paramètre `search` à son hook React Query
- Le backend filtre via `ILIKE '%search%'` sur les champs pertinents (nom, code, email, etc.)
- La recherche se reset automatiquement lors de la navigation (changement de `pathname`)
- `⌘K` / `Ctrl+K` ouvre la **CommandPalette** (recherche globale multi-module)

```
Topbar : [ 🔍 Rechercher — Assets... _______________________ ⌘K ]
                  ↓ globalSearch
Page :    useAssets({ search: globalSearch })
```

### 2. TagSelector — Sélection par tags cliquables

**Règle** : Quand une liste d'options a **≤ 6 éléments**, utiliser des **tags cliquables** au lieu d'un `<select>`.

Le composant `TagSelector` est exporté depuis `@/components/layout/DynamicPanel`.

```tsx
// ✅ Correct — peu d'options → tags
<TagSelector
  options={[
...
// ❌ Incorrect — peu d'options mais utilise un select
<select className="gl-form-select">
  <option value="client">Client</option>
  ...
</select>
```

**Exceptions** : Un `<select>` est acceptable quand :
- Le nombre d'options est dynamique et peut dépasser 6 (ex: liste de parents)
- Les options viennent d'une API (ex: liste d'emails de notification)
- Il y a trop d'options pour un affichage en ligne (ex: liste de pays)

### 3. Formulaire réduit/étendu (Progressive Disclosure)

**Règle** : Les formulaires de création ont **deux niveaux** :
1. **Champs essentiels** — visibles par défaut
2. **Champs optionnels** — cachés derrière un bouton "Plus d'options"

```tsx
// ── Required fields visible by default ──
<FormSection title="Détails">
  <DynamicPanelField label="Code" required>...</DynamicPanelField>
  <DynamicPanelField label="Nom" required>...</DynamicPanelField>
</FormSection>
...
{expanded && (
  <FormSection title="Localisation">
    <DynamicPanelField label="Latitude">...</DynamicPanelField>
  </FormSection>
)}
```

### 4. Édition inline (double-clic)

**Règle** : Dans les panneaux de détail (Dynamic Panel, type `detail`), les champs éditables utilisent le pattern **double-clic pour éditer**.

- `InlineEditableRow` pour les champs texte/email/tel
- `InlineEditableTags` pour les champs à options courtes (type, langue)
- `InlineEditableSelect` pour les champs à options longues
- `ReadOnlyRow` pour les champs non éditables (statut, date de création)

Chaque composant :
- Affiche un icône crayon au survol
- Passe en mode édition au double-clic
- Sauvegarde via Enter ou blur → appel PATCH API → invalidation React Query
- Annule via Escape

### 5. Avatar avec fallback initiales

**Règle** : Partout où un avatar utilisateur est affiché, utiliser le composant `UserAvatar` de `@/components/layout/Topbar`.

- Si `avatar_url` est défini → affiche l'image
- Sinon → affiche les initiales (première lettre prénom + nom)
- L'avatar se met à jour automatiquement via `fetchUser()` après upload

```tsx
import { UserAvatar } from '@/components/layout/Topbar'

<UserAvatar size={24} />  // Topbar
<UserAvatar size={32} />  // Dropdown menu
<UserAvatar size={80} />  // Page profil
```

### 6. Toolbar bar (sous le PanelHeader)

Chaque page liste a une barre d'outils fine (h-9) entre le PanelHeader et le contenu.
Elle contient :
- Toggles de vue (list/tree, cards/table) — à gauche
- Compteur de résultats — à droite
- **Pas de barre de recherche** (elle est dans le Topbar)

```
┌────────────────────────────────────────────────────┐
│ PanelHeader : [icon] Titre          [+ Créer]      │
├────────────────────────────────────────────────────┤
│ Toolbar : [Liste] [Arborescence]          25 total  │
├────────────────────────────────────────────────────┤
│ Table / List content...                             │
└────────────────────────────────────────────────────┘
```

### 7. Adresses polymorphiques — composant unique réutilisable

**Règle** : Les adresses sont gérées via une **table unique `addresses`** avec liaison polymorphique (`owner_type` + `owner_id`).

- **JAMAIS** de champs adresse directement sur les tables objets (tiers, assets, etc.)
- Chaque objet est lié à ses adresses via `owner_type='tier'` + `owner_id=tier.id`
- Le composant `<AddressManager ownerType="tier" ownerId={tier.id} />` est **réutilisable** partout
- Inclut : liste, création inline, modification, suppression, géolocalisation GPS
- Géolocalisation : bouton "Ma position" via `navigator.geolocation.getCurrentPosition()`
- Type d'adresse via `TagSelector` (domicile, travail, site, siège, ramassage, autre)

```tsx
// ✅ Correct — composant partagé, pas de doublon de code
...
<AddressManager ownerType="asset" ownerId={asset.id} compact />
```

### 8. Composants mutualisés obligatoires (Core)

**Règle** : Certains composants/fonctionnalités sont **communs à tous les objets** et doivent être gérés via des tables partagées et des composants réutilisables. **JAMAIS de code dupliqué** entre modules.

| Fonctionnalité | Table DB | Composant frontend | Description |
|---|---|---|---|
| **Adresses** | `addresses` (polymorphique) | `<AddressManager>` | Adresses postales + GPS liées à tout objet |
| **Tags / Catégories** | `tags` (polymorphique) | `<TagManager>` | Classification par tags publics/privés sur tout objet |
| **Notes / Commentaires** | `notes` (polymorphique) | `<NoteManager>` | Notes publiques/privées, historisables, sur tout objet |
| **Fichiers joints** | `attachments` (polymorphique) | `<AttachmentManager>` | Upload, download, suppression. Liés à tout objet |
| **Extra Fields** | `extra_fields` | `<ExtraFieldsManager>` | Champs personnalisés ajoutés dynamiquement |
| **Templates / Modèles** | `form_templates` | `<TemplateSelector>` | Modèles de formulaire réutilisables par utilisateur |

Toutes ces tables utilisent le pattern polymorphique :
```sql
owner_type VARCHAR(50) NOT NULL,  -- 'user', 'tier', 'asset', 'projet', etc.
owner_id   UUID NOT NULL,
INDEX (owner_type, owner_id)
```

#### 8b. Disposition des composants polymorphiques dans les formulaires

Les composants polymorphiques sont organisés en **onglets secondaires** dans le panneau de détail, chaque onglet affiche un **badge compteur** `(N)`.

**Sur la page principale** (toujours visible) : Tags en ligne + Détails + Notes rapides.
**Dans les onglets** (avec badge compteur) : Adresses (N) | Fichiers (N) | Historique | Extra Fields.

#### 8c. Intégrations & Services externes centralisés

Toute config de service externe dans **Settings > Intégrations**. Aucun module ne crée ses propres accès.

#### 8d. Panneau détachable, multi-tâches & indicateur offline

- Dynamic Panel détachable en modal non-bloquant (repositionnable, redimensionnable)
- Système d'onglets applicatif interne pour le multi-traitement
- Modaux superposables pour consulter et remplir simultanément
- Multi-fenêtres navigateur : chaque URL routable en standalone
- **Indicateur LED** TopBar : vert (en ligne) / orange (sync) / rouge (offline)

### 9. Code unique (référence) — numérotation configurable

**Règle** : Le champ `code` de chaque objet (asset, tier, projet, etc.) utilise un **système de numérotation configurable** par le tenant.

- L'utilisateur **NE PEUT PAS saisir librement** le code — il est généré automatiquement
- Le format est défini par un **template configurable** dans les paramètres système (ex: `AST-{YYYY}-{####}`, `TRS-{####}`)
- Le backend utilise `generate_reference(prefix, db)` pour générer la prochaine valeur
- Le champ `code` est affiché en lecture seule dans les formulaires de création
- Le template est configurable dans `Settings > Configuration > Numérotation` pour chaque type d'objet

### 10. RowID — identifiant système immuable

**Règle** : Chaque enregistrement dans le système possède un **rowid** (UUID) généré automatiquement.

- Ce `rowid` est l'identifiant technique interne, **jamais affiché** à l'utilisateur
- C'est la colonne `id` de type UUID v4, générée par le mixin `UUIDPrimaryKeyMixin`
- Le `code` (référence métier) est ce que l'utilisateur voit et utilise
- Les URLs et les API utilisent le `rowid` (UUID) en interne
- Ce rowid est **immuable** — il ne change jamais après la création

### 11. Onglet Notes/Commentaires sur chaque objet

**Règle** : Chaque formulaire de détail (panneau dynamique, type `detail`) comporte un **onglet Notes**.

- Les notes sont stockées dans la table `notes` (polymorphique `owner_type`/`owner_id`)
- Chaque note a : `content` (texte), `is_private` (bool), `created_by` (user), `created_at`
- Les notes sont **historisables** — on ne modifie pas, on ajoute de nouvelles entrées
- Notes publiques : visibles par tous les utilisateurs ayant accès à l'objet
- Notes privées : visibles uniquement par l'auteur
- Le composant `<NoteManager>` est réutilisable partout

### 12. Tags / Catégories mutualisés

**Règle** : Les tags permettent de **classifier et filtrer** tout type d'objet.

- Table `tags` : `owner_type`, `owner_id`, `name`, `color`, `is_private`, `created_by`
- Un tag peut être **public** (visible par tous) ou **privé** (visible uniquement par le créateur)
- La création de tags publics nécessite la permission `tag.create.public`
- Les tags sont utilisables pour filtrer les listes et construire des vues personnalisées
- Le composant `<TagManager>` est réutilisable partout

### 13. Templates de formulaire (Form Templates)

**Règle** : Un utilisateur peut **enregistrer un formulaire rempli comme modèle** pour le réutiliser.

- Table `form_templates` : `owner_type`, `name`, `template_data` (JSON), `created_by`, `is_shared`
- Lors de la création d'un objet, l'utilisateur peut choisir un modèle via `<TemplateSelector>`
- Le modèle pré-remplit tous les champs enregistrés (sauf champs uniques : code, email, etc.)
- Les modèles peuvent être personnels ou partagés (si l'utilisateur a le droit `template.share`)

### 14. Mode plein écran des formulaires

**Règle** : Chaque formulaire (panneau dynamique) dispose d'un **bouton d'agrandissement** qui bascule en vue plein écran.

- En mode normal : le formulaire s'affiche dans le panneau dynamique (240-600px de large)
- En mode plein écran : le formulaire prend toute la zone centrale, en format "boxed" (max-width centré)
- Les champs sont mieux agencés en colonnes (2-3 colonnes au lieu d'une seule)
- Un bouton dans la barre d'actions du panneau permet de basculer entre les deux modes
- Le mode est stocké localement (pas persisté en DB)

---

## 35. Patterns UI implémentés — Référence pratique

> Section ajoutée pour documenter les patterns UI concrètement implémentés et les règles graphiques
> établies pendant le développement. **Toujours suivre ces patterns pour la cohérence.**

---

### 35.1. EmptyState — Composant universel d'état vide

**Fichier** : `src/components/ui/EmptyState.tsx`

**Règle** : Tout écran ou panneau sans contenu utilise `<EmptyState>`. Aucun `<p>` ad hoc.

#### Variantes sémantiques (4)

| Variante | Usage | Icône par défaut | Couleur |
|---|---|---|---|
| `blank` | Aucun contenu encore créé | `Inbox` | `text-muted-foreground/50` |
| `search` | Recherche/filtre sans résultat | `SearchX` | `text-muted-foreground/50` |
| `config` | Fonctionnalité non configurée | `Settings` | `text-primary/50` |
| `error` | Erreur de chargement | `AlertCircle` | `text-destructive/50` |

#### Tailles (3)

| Taille | Usage | Padding | Icône |
|---|---|---|---|
| `compact` | Inline dans un panneau ou tableau | `py-6 px-4` | 20px |
| `default` | Page standard | `py-12 px-6` | 32px |
| `large` | Page hero plein écran | `py-20 px-8` | 48px |
...
<EmptyState
  ...
  action={{ label: 'Nouvelle entreprise', onClick: handleCreate }}
  variant="blank"
  size="default"
/>
```
...
interface EmptyStateProps {
  icon?: LucideIcon           // Icône Lucide (override le défaut de la variante)
  title?: string              // Court (5 mots max, pas de point)
  description?: string        // Phrase explicative complète
  action?: { label: string; onClick: () => void; variant?: 'primary' | 'default' }
  secondaryAction?: { label: string; onClick: () => void }
  variant?: 'blank' | 'search' | 'config' | 'error'
  size?: 'compact' | 'default' | 'large'
  className?: string
  children?: React.ReactNode  // Contenu custom sous la description
}
```

---

### 35.2. Banner — Bannières de découverte et alertes

**Fichier** : `src/components/ui/Banner.tsx`

**Règle** : Toute annonce, alerte contextuelle ou promotion de fonctionnalité utilise `<Banner>`.

#### Variantes (5)

| Variante | Couleur | Usage |
|---|---|---|
| `info` | Bleu | Informations, nouvelles fonctionnalités, tips |
| `success` | Vert | Feedback positif, complétion |
| `warning` | Jaune | Attention, état dégradé, environnement dev |
| `danger` | Rouge | Problème critique, breaking change |
| `promo` | Violet | Mise à niveau, fonctionnalité premium, bienvenue |

#### Fonctionnalités clés

- **Dismissible** : `dismissKey="banner:feature-v1"` → persisté en `localStorage`, ne réapparaît jamais après fermeture
- **Compact** : `compact` prop pour usage inline dans un panneau (padding réduit, texte `text-xs`)
- **Actions** : `action` et `secondaryAction` pour CTA contextuels
- **Illustration** : `illustration` prop pour contenu custom à gauche (sinon icône par défaut)

#### Convention de nommage des dismissKey

```
banner:<feature>-v<version>
```

Exemples : `banner:welcome-v1`, `banner:search-feature-v1`, `banner:dev-env-v1`

Incrémenter la version pour re-montrer une bannière mise à jour.
...
// Bannière de découverte (dismissible)
<Banner
  variant="promo"
  title="Bienvenue sur OpsFlux"
  description="Découvrez les fonctionnalités de votre nouveau ERP."
  action={{ label: 'Découvrir', onClick: () => navigate('/search') }}
  dismissKey="banner:welcome-v1"
/>

// Alerte environnement dev (compact, dismissible)
<Banner
  variant="warning"
...
```typescript
import { resetBannerDismissal } from '@/components/ui/Banner'
resetBannerDismissal('banner:welcome-v1') // Pour tests ou admin
```

---

### 35.3. DangerConfirmButton — Confirmation inline des actions destructives

**Fichier** : `src/components/layout/DynamicPanel.tsx` (exporté)

**Règle** : Toute action destructive (archiver, supprimer, révoquer) utilise `<DangerConfirmButton>`. **Jamais de modal de confirmation** — la transformation inline est le pattern Pajamas.

#### Comportement

1. Premier clic → le bouton se transforme en "Confirmer ?" (texte rouge)
2. Deuxième clic → exécute `onConfirm()`
3. Si pas de deuxième clic → auto-reset après `timeout` ms (défaut 3000)
...
### 35.4. CollapsibleSection — Sections repliables avec deep-linking

**Fichier** : `src/components/shared/CollapsibleSection.tsx`

**Règle** : Dans les pages Settings et les formulaires longs, chaque groupe logique est une `<CollapsibleSection>`.

#### Fonctionnalités

- Animation CSS smooth (transition `max-height`)
- **Deep-link URL** : `#section-id` → auto-expand + scroll-to
- Persistance optionnelle via `localStorage` (`storageKey`)
- Chevron indicateur d'état (rotation animée)
- Wrappeur `<CollapsibleProvider>` pour gérer le contexte global
...
<CollapsibleSection
  id="cartographie"
  title="Cartographie"
  description="Configuration de la carte et du géocodage."
...
#### Deep-link mapping (Settings)

Chaque section enfant est mappée vers son onglet parent dans `SECTION_TAB_MAP` :
...
URL `/settings#cartographie-integration` → active l'onglet `integrations` puis expand + scroll vers la section.

---

### 35.5. TagManager hiérarchique — Tags parent/enfant en arbre

**Fichier** : `src/components/shared/TagManager.tsx`

**Règle** : Le TagManager affiche les tags en **arbre hiérarchique** (parent/enfant), pas en liste plate.

#### Architecture

- Hook `useTagTree(ownerType, ownerId)` → retourne l'arbre structuré `TagTree[]`
- Composant récursif `TagTreeNode` pour le rendu en profondeur
- Indentation : `marginLeft: depth * 16px`
- Chevron expand/collapse sur les parents
- Icône `FolderTree` sur les nœuds parents

#### Sélecteur de parent

Le formulaire de création et d'édition inline inclut un **sélecteur de parent** :

```tsx
<select value={parentId} onChange={...}>
  <option value="">— Aucun parent —</option>
  {flatTags.filter(t => t.id !== currentTag?.id).map(t => (
...
Le hook `useCreateTag` invalide **toutes les queries tag** (pas seulement la liste plate) :

```typescript
onSuccess: () => {
  qc.invalidateQueries({ queryKey: ['tags'] }) // ← invalide tree + flat
}
```

---

### 35.6. Badge compteur sur les items sidebar (Settings)

**Fichier** : `src/hooks/useSettingsBadges.ts` + `src/pages/settings/SettingsPage.tsx`

**Règle** : Les items de la sidebar Settings affichent un **badge compteur** (pill) pour les sections où un comptage est pertinent.

#### Sections avec badge

| Section ID | Source | Description |
|---|---|---|
| `tokens` | `tokensService.list()` | Tokens d'accès API |
| `applications` | `oauthAppsService.list()` | Applications OAuth2 |
| `sessions` | `sessionsService.list()` | Sessions actives |
| `emails` | `emailsService.list()` | Adresses email |
| `roles` | `rolesService.getUserRoles()` | Rôles/groupes |
| `users` | `usersService.list()` | Utilisateurs (admin) |
| `assets` | `assetsService.list()` | Assets (admin) |
| `email-templates` | API `/api/v1/email-templates` | Templates email |
...
- Fond : `bg-accent`
- Forme : `rounded-full`
- Couleur texte : `text-muted-foreground`
- Padding horizontal : `px-1.5`
- Largeur minimum : `min-w-[18px]`

#### Performance

- `staleTime: 60_000` (1 minute) pour éviter les requêtes excessives
- Endpoints paginés appellent `page_size: 1` et lisent `data.total`

#### Piège CSS résolu

Quand un `<span>` a `flex-1` dans un `<button>`, le texte se centre (car `<button>` = `text-align: center` par défaut). **Toujours ajouter `text-left`** :

```tsx
// ✅ Correct
<span className="truncate flex-1 text-left">{label}</span>

// ❌ Incorrect — texte centré involontairement
<span className="truncate flex-1">{label}</span>
```

---

### 35.7. ConnectorStatus — Statut de connexion des services externes (4 états)

**Fichier** : `src/pages/settings/tabs/IntegrationsTab.tsx`

**Règle** : Ne pas confondre **configuré** et **connecté**. Un service est "configuré" quand les champs sont remplis, mais "connecté" seulement après un test de connexion réussi.

#### Les 4 états

| État | Signification | Style badge | Style bordure carte |
|---|---|---|---|
| `idle` | Aucune configuration | zinc-100/zinc-500 | `border-border/60` |
| `configured` | Champs remplis, non testé | amber-100/amber-700 | `border-amber-300` |
| `connected` | Test de connexion réussi | emerald-100/emerald-700 + ✓ | `border-emerald-300` |
| `error` | Test de connexion échoué | red-100/red-700 + ⚠ | `border-red-300` |
...
#### Backend — Test de connexion

**Endpoint** : `POST /api/v1/integrations/test`

```python
class TestRequest(BaseModel):
    connector_id: str  # Ex: "smtp", "s3_storage", "google_oauth"
...
Testeurs spécifiques par connecteur :
- `smtp` → `aiosmtplib.connect()` + auth
- `s3_storage` → `boto3.head_bucket()`
- `webhook` → `httpx.head(url)`
- `sms_twilio` → API `/2010-04-01/Accounts/{sid}`
- `sms_vonage` → API `/account/get-balance`
- `google_oauth`, `azure_ad`, etc. → validation de la config

Les résultats sont persistés comme settings :
- `integration.<connector_id>.last_test_status` → `"ok"` | `"error"`
- `integration.<connector_id>.last_test_at` → ISO datetime
- `integration.<connector_id>.last_test_error` → message d'erreur

#### Barre récapitulative

En bas de la section "Services connectés", une barre affiche le résumé : `"X connecté(s), Y configuré(s), Z en erreur sur N ajouté(s)"`.

---

### 35.8. Recherche globale — Architecture TopBar → Page dédiée

**Règle** : La recherche est accessible **uniquement via la TopBar** (pas de lien sidebar). Appuyer sur Entrée dans le champ de recherche navigue vers `/search?q=...`.

#### Flux utilisateur

```
TopBar Search → saisie → Entrée → navigate('/search?q=keyword')
TopBar Search → ⌘K → CommandPalette → raccourcis/navigation rapide
```

#### Page `/search` (SearchPage)

**Fichier** : `src/pages/search/SearchPage.tsx`

- **Onglets de scope** : Tout | Assets | Tiers | Utilisateurs
- **État URL** : `useSearchParams` (`?q=keyword&scope=asset&page=1`)
- **Sync initiale** : récupère `globalSearch` du `uiStore` au montage
- **Debounce** : 300ms sur la saisie
- **Résultats paginés** : badges colorés par type, lignes cliquables
- **i18n** : toutes les chaînes via `t('search.*')`

#### Pourquoi pas de sidebar item

L'utilisateur accède à la recherche avancée via :
1. Saisie + Entrée dans la TopBar
2. Le bouton "Recherche avancée" dans la CommandPalette

Un lien sidebar serait redondant.

---

### 35.9. Lazy Loading des routes — React.lazy + Suspense

**Fichier** : `src/App.tsx`

**Règle** : Toute page-level route est chargée en lazy. Seule `LoginPage` est importée directement (critique path).
...
#### Note `.then(m => ({ default: m.SettingsPage }))`

Nécessaire quand le composant est un **named export** (pas `export default`). Convertit en default export pour `React.lazy`.

---

### 35.10. Vite — Chunk splitting avec manualChunks

**Fichier** : `vite.config.ts`

**Règle** : Les dépendances lourdes sont isolées dans des chunks nommés pour optimiser le cache navigateur.
...
| Chunk | Contenu | Raison |
|---|---|---|
| `react-vendor` | React, ReactDOM, Router | Change rarement, cache long |
| `query` | React Query | Dépendance lourde isolée |
| `ui-icons` | Lucide React | ~200+ icônes, cache long |
| `i18n` | i18next + react-i18next | Change rarement |

Combiné avec `React.lazy`, chaque page est un chunk séparé automatiquement.

---

### 35.11. i18n — Convention d'internationalisation

**Fichiers** : `src/locales/fr/common.json`, `src/locales/en/common.json`

**Règle** : **Toute chaîne UI** passe par `t()` de `react-i18next`. Aucun texte en dur dans les composants (sauf `console.log` et commentaires).

#### Convention des clés

```
<namespace>.<contexte>.<element>
```
...
#### Piège : shadowing de `t`

Ne jamais nommer une variable `t` si `useTranslation` est utilisé :

```typescript
// ❌ Conflit : t masqué par setTimeout
const t = setTimeout(...)
...
### 35.12. Settings Registry — Enregistrement extensible des onglets

**Fichier** : `src/lib/settingsRegistry.ts`

**Règle** : Les onglets Settings sont enregistrés via un registre, pas hardcodés dans SettingsPage. Tout module peut ajouter ses propres sections.
...
registerSettingsSection({
  ...
  label: 'Intégrations',
  icon: Plug,
  category: 'general',           // 'user' | 'general'
  order: 80,                      // Tri dans la sidebar
  component: () => import('@/pages/settings/tabs/IntegrationsTab').then(m => m.IntegrationsTab),
})
...
registerSettingsGroup({
  ...
  category: 'user',
  order: 30,
  children: ['tokens', 'applications', 'sessions'],
})
```

#### Catégories

| Catégorie | Sidebar | Description |
|---|---|---|
| `user` | Section haute | Paramètres personnels (profil, sécurité, préférences) |
| `general` | Section basse | Administration système (rôles, utilisateurs, intégrations) |

#### Groupes collapsibles

Les sections peuvent être groupées sous un parent collapsible (ex: "Accès" contient Tokens + Applications + Sessions). Le groupe affiche un chevron et ses enfants sont indentés.

---

### 35.13. Règles CSS récurrentes — Aide-mémoire

#### Boutons sidebar/nav

```css
/* Item de sidebar Settings */
button.h-7.w-full.rounded-md.flex.items-center.gap-2.px-2.text-sm.transition-colors
...
/* Label avec badge — toujours text-left */
span.truncate.flex-1.text-left
```

#### Badges compteurs (pill)

```css
/* Badge dans sidebar */
text-[10px] text-muted-foreground bg-accent rounded-full px-1.5 min-w-[18px] text-center

/* Badge dans notification bell */
absolute -top-0.5 -right-0.5 h-4 w-4 rounded-full bg-destructive text-destructive-foreground text-[10px]

/* Badge dans catégorie de connecteurs */
text-[10px] text-muted-foreground bg-accent rounded-full px-1.5
```

#### Cartes de connecteur (bordure colorée)
...
```css
/* Input standard */
gl-form-input text-sm w-full

/* Input monospace (API keys, codes) */
gl-form-input text-sm w-full font-mono
...
### 35.14. Résumé — Checklist "Nouveau composant"

Avant de créer un nouveau composant ou pattern, vérifier cette liste :

```
□ État vide → utiliser <EmptyState> (pas de <p> ad hoc)
□ Alerte/annonce → utiliser <Banner> (pas de div colorée ad hoc)
□ Action destructive → utiliser <DangerConfirmButton> (pas de modal)
□ Section repliable → utiliser <CollapsibleSection>
□ Tags → utiliser <TagManager> (hiérarchique)
□ Notes → utiliser <NoteManager>
□ Adresses → utiliser <AddressManager>
□ Fichiers joints → utiliser <AttachmentManager>
□ Champs perso → utiliser <ExtraFieldsManager>
□ Toutes les chaînes UI → t() (i18n)
□ Routes page-level → React.lazy + Suspense
□ Settings tabs → registerSettingsSection()
□ Nouveau service externe → 4 états (idle/configured/connected/error)
□ Sidebar label avec flex → text-left explicite
□ Badge compteur → pill text-[10px] bg-accent rounded-full
```
