...
class UserDashboardTab(UUIDPrimaryKeyMixin, Base):
    ...
    __table_args__ = (Index("idx_user_dashboard_tabs_user_entity", "user_id", "entity_id"),)
...
class Dashboard(UUIDPrimaryKeyMixin, SoftDeleteMixin, Base):
    ...
    bu_id: Mapped[PyUUID | None] = mapped_column(
        UUID(as_uuid=True),
        nullable=True,  # BU scope (no FK — managed at app level, nullable=True
    )
    ...
    nav_show_in_sidebar: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", nullable=False)
    ...
    tv_token_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class DashboardPermission(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    inherit_from_parent: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false", nullable=False)
