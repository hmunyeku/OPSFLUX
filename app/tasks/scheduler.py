...
def _on_job_submitted(event) -> None:
    _job_start_times[event.job_id] = datetime.now(UTC)
...
async def log_manual_execution(
    job_id: str,
    job_name: str,
    started_at: datetime,
...
def _register_jobs() -> None:
    ...
    from app.core.scheduler import generate_recurring_activities_job, refresh_daily_pax_load_job
    ...
    scheduler.add_job(
        ...
        trigger=CronTrigger(hour=3, minute=0),
        id="gdpr_purge",
        name="RGPD: purge donnees hors retention",
        replace_existing=True,
        max_instances=1,
    )
    ...
    scheduler.add_job(
        check_asset_inspections,
        trigger=CronTrigger(hour=7, minute=0),
        id="asset_inspection",
        name="Rappels inspections assets",
        replace_existing=True,
        max_instances=1,
    )
    ...
    scheduler.add_job(
        process_overdue_ads_closure,
        trigger=CronTrigger(hour=1, minute=30),
        id="paxlog_overdue_ads_closure",
        name="Alerter et clôturer les AdS en dépassement",
        replace_existing=True,
        max_instances=1,
    )
    ...
    scheduler.add_job(
        process_requires_review_followup,
        trigger=CronTrigger(hour=1, minute=45),
        id="paxlog_requires_review_followup",
        name="Rappeler les AdS bloquées en nécessite révision",
        replace_existing=True,
        max_instances=1,
    )
