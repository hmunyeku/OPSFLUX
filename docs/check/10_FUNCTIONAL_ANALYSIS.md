# OpsFlux — 10_FUNCTIONAL_ANALYSIS.md
...
- Le tenant actif est encodé dans le JWT (`tenant_id` claim)
- Pour switcher de tenant : nouvel échange token avec `tenant_id` différent
...
User clique sur Tenant Switcher → voit liste de ses tenants
  → Clique sur un tenant → request refresh token avec nouveau tenant_id
...
User A partira en congés (admin ou reviewer)
  ...
  → À l'expiration : délégation désactivée automatiquement (job ARQ)
```
...
| Deadline dans 48h | Haute | Job ARQ check_workflow_deadlines | Bouton "Traiter" |
| Deadline dépassée | Critique | Job ARQ | Bouton "Voir" |
...
- `tenant_id = :tenant_id` injecté obligatoirement si tables OpsFlux
