...
def _default_workflow_definitions() -> list[dict]:
    return [
        ...
        {
            "slug": "travelwiz-cargo-workflow",
            "legacy_slugs": [],
            "name": "TravelWiz Cargo Workflow",
...
async def seed_dashboard_tabs(db: AsyncSession, entity_id) -> None:
    ...
    ROLE_TABS: list[dict] = [
        ...
        # Conformité module dashboard
        {
            ...
            "tab_order": 0,
            "widgets": [
                _make_widget("conformite_kpis", "KPIs Conformite",
                             {"source": "conformite"},
                             {"x": 0, "y": 0, "w": 4, "h": 2}),
                _make_widget("paxlog_compliance_rate", "Taux de conformite PAX",
                             {"source": "paxlog"},
                             {"x": 4, "y": 0, "w": 4, "h": 2}),
                _make_widget("alerts_urgent", "Alertes",
                             {"source": "core"},
                             {"x": 8, "y": 0, "w": 4, "h": 2}),
                _make_widget("conformite_by_category", "Par categorie",
                             {"source": "conformite", "chart_type": "bar"},
                             {"x": 0, "y": 2, "w": 6, "h": 4}),
                _make_widget("compliance_expiry", "Certifications expirant 30j",
                             {"source": "paxlog", "days_ahead": 30},
                             {"x": 6, "y": 2, "w": 6, "h": 4}),
