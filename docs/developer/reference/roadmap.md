...
| DO | Redis démarré et connecté à ARQ | Worker ARQ démarre sans erreur | 0.5j |
| DO | Intégrer OAuth2/OIDC avec l'Identity Provider Perenco (protocole OIDC) | Login via SSO → token JWT avec user_id + tenant_id → API répond 200 | 3j |
...
| Étape | Tâche détaillée | Critère mesurable | Effort |
|---|---|---|---|
| PLAN | Définir le JSON schema complet d'un template (voir 02_MODULE_REPORT.md §4) | Schema documenté dans le code (Pydantic TemplateSchema) | 1j |
...
| DO | Deadlines : job ARQ `check_workflow_deadlines` → alertes J-2 et J+0 | Recommandation "Validation en retard" créée à J+1 | 2j |
...
| DO | Merge CRDT offline → online (cas edge documentés dans 02_MODULE_REPORT.md §6) | User offline 30 min + user online modifient la même section → merge propre | 3j |
...
| PLAN | Définir le catalogue complet des tools MCP (voir 07_AI_MCP.md §3) | Liste de 15 tools documentés avec permissions | 1j |
...
| DO | Pipeline indexation : document publié → ARQ job → chunking 600 tokens → embedding → pgvector | Document publié → vecteurs stockés en < 30s, vérifiés en DB | 4j |
