...
export function ManageMembersDialog({
  ...
  const loadData = async () => {
    try {
      ...
      const membersResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/roles/${roleId}/users`, {
        headers,
      })
  ...
  const handleSave = async () => {
    try {
      ...
      // Update role members
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/roles/${roleId}/users`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      toast({
        ...
        description: t("success.members_updated_desc", "Les membres du rôle ont été mis à jour avec succès"),
      })
      ...
      <SheetContent className="sm:max-w-xl">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            ...
            {t("dialog.manage_members_title", "Gérer les utilisateurs")}
          </SheetTitle>
          <SheetDescription>
            {t("dialog.manage_members_desc", "Ajoutez ou retirez des utilisateurs du rôle")} <strong>{roleName}</strong>
          </SheetDescription>
        </SheetHeader>
